...
"""
save_txt: False 
not generate "predict" folder when training
"""
label_idx_names = {}
merge_labels = True
# weed2021  weed2022 weed_all weed_lambsquarters weed_stable_diffusion weed_stable_diffusion_controlnet
# weed_stable_diffusion_IPAdapter weed_semi_supervised
# blueberry_dense cottonboll_dense weed2023 COCO weed2021_3sku
# target_name = 'weed_stable_diffusion_controlnet'
target_name = 'weed_stable_diffusion_IPAdapter'
merge_labels_dict = {}
...
if is_debug:
    ...
    step_num_per_epoch = 20
    
is_val = False
use_date = True
if is_debug:
    use_date = False
        # if y.__class__.__name__ == 'IterableSimpleNamespace':
        #     return
verbose = True
...
not_show_label = True and is_val
plot_all_eval_images = False and is_val
plot_each_sku_separately = False and is_val

use_large_label_size = False
...
show_recal_per_im = False and is_val
# show certain bboxes
...
plot_maturity_rate_and_counting = True and is_val and target_name == 'blueberry_dense'
...
if target_name == 'blueberry_dense':
    ...
    # plot_bbox_conf = 0.25
    # val_bbox_conf = 0.001
    # pred_bbox_conf = 0.001
else:   
    # default conf=0.25 0.001
    plot_bbox_conf = 0.25
    # default.yaml
    # val_bbox_conf = 0.05
    val_bbox_conf = 0.001
...
control_the_conf_t_for_p_and_r = False and is_val
if control_the_conf_t_for_p_and_r:
    controled_the_conf_t = 0.2
...
def show_image_with_bbox(x, targets=None, colors=None, label_idx=5, conf_idx=4, use_cxcywh=False, font_scale=0.5, font_thickness=2, name='x', save=False, show=True, rgb2bgr=False):
    ...
    if targets is not None:
        ...
        for i_y in range(bbox_num):
            ...
            conf = round(targets_[i_y, conf_idx], 2) 
            conf = round(conf*100)
            bbox_start_idx = 0
            bbox_end_idx = 4
            ...
            color = (0, 128, 255)
            cv2.putText(z, str(conf)+'%', (int(x0), int(y0)-5), cv2.FONT_HERSHEY_SIMPLEX, font_scale, color, thickness)
            # cv2.putText(z, str(sku_id), (int(x0)  + 50, int(y0)), cv2.FONT_HERSHEY_SIMPLEX, font_scale, color, thickness)
...
if compare_original_prediction:
    log_xalg('', log_path=r'D:\test\_update_box_precision_per_im.txt', overwrite=True)
    log_xalg('', log_path=r'D:\test\_new_box_precision_per_im.txt', overwrite=True)
    log_xalg('', log_path=r'D:\test\_original_precision_per_im.txt', overwrite=True)
    log_xalg('', log_path=r'D:\test\_optimized_precision_per_im.txt', overwrite=True)
