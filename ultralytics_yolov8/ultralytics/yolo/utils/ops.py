...
def non_max_suppression(
    ...
    time_limit = 1.0 + max_time_img * bs  # seconds to quit after
...
def non_max_suppression_1(
    ...
    Arguments:
        prediction (torch.Tensor): A tensor of shape (batch_size, num_boxes, num_classes + 2)
            containing the predicted boxes, classes, and masks. The tensor should be in the format
            output by a model, such as YOLO.
        conf_thres (float): The confidence threshold below which boxes will be filtered out.
            Valid values are between 0.0 and 1.0.
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        # x[((x[:, 2:4] < min_wh) | (x[:, 2:4] > max_wh)).any(1), 4] = 0  # width-height
        x = x[xc[xi]]
        #x = x.transpose(0, -1)  # confidence

        # Detections matrix nx6 (xyxy, conf, cls)
        box, conf_, cls = x.split((4, 1, 1), 1)
        #box = xywh2xyxy(box)  # center_x, center_y, width, height) to (x1, y1, x2, y2)

        # best class only
        #conf, j = conf_.max(1, keepdim=True)
