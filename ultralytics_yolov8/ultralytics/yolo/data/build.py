...
def build_dataloader(cfg, batch, img_path, stride=32, rect=False, names=None, rank=-1, mode='train', domain_adaptation=False, need_label=True):
    ...
    with torch_distributed_zero_first(rank):  # init dataset *.cache only once if DDP
        dataset = YOLODataset(
            ...
            imgsz=cfg.imgsz,
            batch_size=batch,
            augment=mode == 'train' and not domain_adaptation,  # augmentation
            hyp=cfg,  # TODO: probably add a get_hyps_from_cfg function
            rect=cfg.rect or rect,  # rectangular batches
            cache=cfg.cache or None,
            single_cls=cfg.single_cls or False,
            stride=int(stride),
            pad=0.0 if mode == 'train' else cfg_tot.val_pad,
