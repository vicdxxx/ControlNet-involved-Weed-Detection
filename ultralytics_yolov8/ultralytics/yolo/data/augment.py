...
class BaseMixTransform:
    ...
    def _mix_transform(self, labels, use_tensor=False):
...
class Mosaic(BaseMixTransform):
    ...
    def _mix_transform(self, labels, use_tensor=False):
        ...
        for i in range(4):
            ...
            if i == 0:  # top left
                if use_tensor:
                    assert img.max() <= 1.0
                    img4 = torch.full((s * 2, s * 2, img.shape[2]), 0.5, dtype=torch.float32)  # base image with 4 tiles
                else:
    ...
    def _cat_labels(self, mosaic_labels):
        ...
        if len(instances) > 0:
            all_instances = Instances.concatenate(instances, axis=0)
        else:
...
class MixUp(BaseMixTransform):
    ...
    def _mix_transform(self, labels, use_tensor=False):
        ...
        if use_tensor:
            assert labels['img'].max() <= 1.0
            labels['img'] = (labels['img'] * r + labels2['img'] * (1 - r))
        else:
            labels['img'] = (labels['img'] * r + labels2['img'] * (1 - r)).astype(np.uint8)
        if isinstance(labels['instances'], Instances):
...
class LetterBoxPadRightBottom:
    """Resize image and padding for detection, instance segmentation, pose"""

    def __init__(self, new_shape=(640, 640), auto=False, scaleFill=False, scaleup=False, stride=32):
        self.new_shape = new_shape
        self.auto = auto
        self.scaleFill = scaleFill
        self.scaleup = scaleup
...
class Albumentations:
    ...
    def __init__(self, p=1.0):
        ...
        try:
            ...
            if cfg_ss.use_stronger_Albumentations:
                T = [
                    A.Blur(p=0.3),
                    A.MedianBlur(p=0.3),
                    A.ToGray(p=0.1),
