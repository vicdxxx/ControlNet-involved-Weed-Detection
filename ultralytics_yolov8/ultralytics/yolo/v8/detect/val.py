...
class DetectionValidator(BaseValidator):
    ...
    def preprocess(self, batch):
        ...
        nb = len(batch['img'])
        self.lb = []
        if 'cls' in batch and 'bboxes' in batch and 'batch_idx' in batch:
            if not isinstance(batch['cls'], np.ndarray):
    ...
    def postprocess(self, preds, conf=None, iou=None):
        if conf==None:
            conf = self.args.conf
        if iou==None:
            iou = self.args.iou
    ...
    def update_metrics(self, preds, batch):
        ...
        for si, pred in enumerate(preds):
            ...
            if not self.training:  # compute recall_ and precision need per sku?
                correct_bboxes_mAP50 = correct_bboxes[:, 0]
                if correct_bboxes_mAP50 is not None:
                    ...
                    if cfg.show_recal_per_im:
                        try:
                            im_name = os.path.basename(cfg.current_im_name[0])
                        except Exception as e:
                            im_name = 'unknown'
                            import traceback
                        ...
                        try:
                            cfg.log_xalg('')
                            cfg.log_xalg(im_name)
                            for sku in torch.unique(cls):
                                print(sku.cpu().numpy(), 'gt number:', len(cls[(cls==sku).squeeze()]))
                                print(sku.cpu().numpy(), 'prediction number:', len(correct_bboxes_mAP50[(pred[:, 5]==sku).squeeze()]))
                                recall_sku = round(torch.sum(correct_bboxes_mAP50[(pred[:, 5]==sku).squeeze()]).cpu().numpy() / len(cls[(cls==sku).squeeze()]), 2)
                                precision_sku = round(torch.sum(correct_bboxes_mAP50[(pred[:, 5]==sku).squeeze()]).cpu().numpy()/max(len(correct_bboxes_mAP50[(pred[:, 5]==sku).squeeze()]), 1), 4)
                                cfg.log_xalg(sku.cpu().numpy(), 'recall:', recall_sku, 'precision:', precision_sku)
                        except Exception as e:
                            print(e)
                            import traceback
                            print(traceback.format_exc())
                    ...
                    log_path= os.path.join(self.save_dir, 'optimized_precision_per_im.txt')
                    f_log = open(log_path, 'a')
                    f_log.close()

                    corret_box_num = torch.sum(correct_bboxes_mAP50).cpu().numpy()
                    tot_recall = round(accumulate_finds / accumulate_GT, 2)
                    precision50 = round(torch.sum(correct_bboxes_mAP50).cpu().numpy()/max(len(correct_bboxes_mAP50), 1), 4)
                    if cfg.show_recal_per_im:
                        cfg.log_xalg('gt box num:', len(cls), 'pred box num:', len(correct_bboxes_mAP50),
                            'corret box num:', corret_box_num, batch['img'].shape, log_path=log_path, overwrite=False, show=True)
    ...
    def _process_batch(self, detections, labels, pre_name=''):
        ...
        if cfg.show_recal_per_im and cfg.target_name == 'blueberry_dense':
            correct_class_unblue = labels[:, 0:1] == 0
            correct_class_blue = labels[:, 0:1] == 3
    ...
    def plot_val_samples(self, batch, ni):
        if cfg.plot_each_sku_separately:
            cls_list = torch.unique(batch['cls'])
            assert len(torch.unique(batch['batch_idx']))==1
            for sku in cls_list:
                idx = batch['cls'].squeeze(-1) == sku
                plot_images(batch['img'],
                            batch['batch_idx'][idx],
                            batch['cls'].squeeze(-1)[idx],
                            batch['bboxes'][idx],
                            paths=batch['im_file'],
    ...
    def plot_predictions(self, batch, preds, ni):
        if cfg.plot_each_sku_separately:
            assert len(torch.unique(batch['batch_idx']))==1
            batch_idxes, skus, bboxes = output_to_target(preds, max_det=cfg.max_det)
            cls_list = np.unique(skus)
            for sku in cls_list:
                idx = skus == int(sku)
                plot_images(batch['img'],
                            batch_idxes[idx],
